...
public class CharacterStats : MonoBehaviour
{
    ...
    public virtual void DoMagicalDamage(CharacterStats _targetStats)
    {
        ...
        float totalMagicalDmg = intelligence.GetValue();

        if (Mathf.Max(_fireDamage, _iceDamage, _shockDamage) <= 0)
        {
            totalMagicalDmg = CheckTargetMagicalResistance(_targetStats, totalMagicalDmg);
            _targetStats.TakeDamage(totalMagicalDmg);
            return;
        }

        AttemptToApplyAilment(_targetStats, _fireDamage, _iceDamage, _shockDamage, totalMagicalDmg);
    ...
    private void AttemptToApplyAilment(CharacterStats _targetStats, float _fireDamage, float _iceDamage, float _shockDamage, float totalMagicalDamge)
    {
        ...
        if (!canApplyIgnite && !canApplyChill && !canApplyShock)  // Ô­±¾ÊÇ while
        {
    ...
    private void HitClosestTargetWithShockStrike()
    {
        ...
        if (closestEnemy != null)
        {
            ...
            GameObject newShockStrike = ObjectPoolManager.instance.getPooledObject(shockStrikePrefab, transform.position, Quaternion.identity);
    ...
    {
        float totalCritPower = GetTotalCritPower() * .01f;
